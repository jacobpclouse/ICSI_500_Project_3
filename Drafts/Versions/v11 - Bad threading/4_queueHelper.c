...
void *serverDecoder(void *arg)
{
    ...
    */
    ...
    int currentThreadIDCASE = threadData->threadCountCase;
...
// ----
        ...
        for (int i = 0; i < num_queues; i++)
        {
            ...
            pthread_create(&firstThread, NULL, serverDecoder, (void *)&threadData);
...
/*
            ...
            threadData.threadCountCase = threadCaseSwitcher;

            // Create a thread
            pthread_t thread;
            if (pthread_create(&thread, NULL, serverDecoder, (void *)&threadData) != 0)
            {
